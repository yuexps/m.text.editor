...
- **类型**: [修复] / [优化]
- **受影响模块**: 前端 UI / CSS 样式 / JS 逻辑层 / 规约文档
- **变更明细**:
  - **修复移动端软键盘遮挡主编辑区与底部终端缺陷**：在 `./build/app/www/js/ui/manager.js` 中收敛并常驻注册 `setupVisualViewportListener`，在软键盘弹出时将根布局 `.vscode-layout` 高度动态收缩至可视视口高度，使状态栏贴合键盘上沿；增加 `visualViewport.scale` 缩放校验（`scale ≈ 1`），杜绝 PC 触屏设备高倍缩放时的误判；仅当布局高度发生跳变时才触发 `TerminalManager.resize()` 与 `editor.layout()`，避免动画期间重复重排；为 `.bottom-panel`、`.panel-body`、`.terminal-body-container` 补充 `min-height: 0`，使底部面板及终端显示区随软键盘顶高的可视视口自由收缩，辅助按键与终端文字始终保持在键盘上方。
  - **架构解耦与冗余清理**：将全局视口监听逻辑从 `./build/app/www/js/ui/sidebar.js` 解耦归位至 `UIManager`，清理侧栏中冗余的 `sidebar.style.bottom` 赋值与潜在样式冲突，使 `sidebar.js` 严格符合单一职责契约。
  - **移动端开放文件管理器定位**：修改 `./build/app/www/js/fnos_sdk.js`，取消移动端下对侧边栏“在 FNOS 文件管理器中定位”按钮（`#open-file-manager-btn`）的强制隐藏，在 FNOS 容器环境下全端统一可用。
  - **终端按键拦截与安全剪贴板注入优化**：在 `./build/app/www/js/terminal.js` 中通过 `attachCustomKeyEventHandler` 精准拦截 `Ctrl+V` / `Cmd+V` / `Shift+Insert`，阻断向 PTY 误发 `\x16`（LNEXT）导致终端进入字面量转义模式的乱码根因；接入统一 `paste` 原生事件与 `terminalInstance.paste()` 安全管道，自动适配括号粘贴与换行符转换；支持 `Ctrl+C` / `Cmd+C` 选区智能感知复制；在触屏工具栏 `Ctrl` 修饰模式下拦截 `V` 自动映射为剪贴板安全读取注入。
  - **补充规约文档**：在 `./docs/AGENT_QUICKREF.md` 中新增第 3.10 节“终端按键拦截与剪贴板注入契约”。
