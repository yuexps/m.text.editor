...
- **变更明细**:
  ...
  - **新增终端 Git 常用命令快捷菜单**：右上角引入 Git 菜单按钮，预置 8 个按常用频次排序且带淡色中文注释的 Git 常用指令，点击仅填入文本支持二次编辑。修复了因样式继承导致高度折叠为 0 的定位 Bug，并于 `./build/app/www/css/responsive.css` 中追加窄屏和触控适配。
