...
cd src/
# Linux amd64
GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -ldflags="-s -w" -o ../build/app/server/podnote .

# Linux arm64
GOOS=linux GOARCH=arm64 CGO_ENABLED=0 go build -ldflags="-s -w" -o ../build/app/server/podnote .
```
...
|------|------|
| `appname` | `podnote` |
| `version` | 语义版本号（如 `1.3.36`） |
| `display_name` | `PodNote` |
| `platform` | `x86` 或 `arm` |
| `maintainer` | 维护者 |
| `desktop_uidir` | `ui`（UI 配置目录） |
| `desktop_applaunchname` | `podnote.open` |
...
| 源码路径 | 容器路径 | 说明 |
|----------|----------|------|
| `build/app/www/` | `/app/podnote/www/` | 前端静态资源 |
| `build/app/server/` | `/app/podnote/server/` | Go 二进制 |
| `build/app/ui/` | `/app/podnote/ui/` | UI 配置与图标 |
...
2. 查询 `app_service` 表中 `service_name = 'podnote.custom'` 的原有后缀
3. 合并新老后缀并去重
...
| 服务名 | 说明 | 默认后缀 |
|--------|------|----------|
| `podnote.open` | 主关联 | txt, md, json, html, css, js, ts, py, sh, yml, go, java 等 68 种 |
| `podnote.custom` | 用户自定义 | key, pem, crt（可通过向导修改） |
