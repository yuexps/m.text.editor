...
4. **开发习惯红线**：
   ...
   - 前端虚拟协议路径（如主页的 `podnote://welcome`）严禁穿透至底层物理 I/O 操作及后端 Go 文件读写接口，必须在前端各业务层进行过滤与防护。
...
3. **接口与事件契约约束**：涉及模块间通信或前后端数据交互时，优先查阅 [docs/BACKEND_API.md](./docs/BACKEND_API.md) 及 [docs/EVENT_BUS.md](./docs/EVENT_BUS.md)。严禁通过分析具体业务 Handler 的实现细节来逆向推演接口格式或通信载荷，确保架构解耦与信息一致性。
...
1. **定位并阅读规范**：在修改前，根据下方的“模块地图”定位目录，优先阅读对应的 `README.md` 与 [docs/AGENT_QUICKREF.md](./docs/AGENT_QUICKREF.md) 中的路径约束与常见陷阱，理解职责划分。
2. **编码与注释规范**：
...
* **技术文档**：[docs/INDEX.md](./docs/INDEX.md) — Agent 与开发者使用的完整技术文档（架构、API、模块、事件协议、构建部署等）。

---

## 6. CHANGELOG.md 变更日志规范
